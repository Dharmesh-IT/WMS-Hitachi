...
public class AdoConnection : IAdoConnection
{
    ...
    public DataTable bulkImport(string SPName, DataTable dt, DateTime recvDate, string loginBranch)
    {
        DataTable SqlDataTable = null;
        ...
        if (!string.IsNullOrEmpty(SPName))
        {
            using (IDbConnection Connector = GetConnection())
            {
                ...
                cmd.Parameters.AddWithValue("@recvDate", recvDate.ToString("yyyy-MM-dd HH:mm:ss"));
                cmd.Parameters.AddWithValue("@loginBranch", loginBranch);
                try
                {
                    Connector.Open();
                    cmd.ExecuteNonQuery();
    ...
    public DataTable bulkImport(string SPName, DataTable dt, DataTable dtSerialMapping, DateTime recvDate, string loginBranch,string companyId)
    {
        DataTable SqlDataTable = null;
        ...
        if (!string.IsNullOrEmpty(SPName))
        {
            using (IDbConnection Connector = GetConnection())
            {
                ...
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@tblTypeIntransit", dt);
                cmd.Parameters.AddWithValue("@tblTypeSerialMapping", dtSerialMapping);
                cmd.Parameters.AddWithValue("@recvDate", recvDate.ToString("yyyy-MM-dd HH:mm:ss"));
                cmd.Parameters.AddWithValue("@loginBranch", loginBranch);
                cmd.Parameters.AddWithValue("@companyName", companyId);
                try
                {
                    Connector.Open();
                    cmd.ExecuteNonQuery();
